...
module BitsService
  class SafeZipper
    ...
    def unzip!
      raise Error.new('Destination does not exist') unless File.directory?(@zip_destination)
      raise Error.new('Relative path(s) outside of root folder') if any_outside_relative_paths?

      unzip

      raise Error.new('Symlink(s) point outside of root folder') if any_outside_symlinks?

      size
    end

    def zip!
      raise Error.new('Path does not exist') unless File.exist?(@zip_path)
      raise Error.new('Path does not exist') unless File.exist?(File.dirname(@zip_destination))
    ...
    def zip
      @zip ||= begin
        ...
        unless status.success?
          raise Error.new("Could not zip the package\n STDOUT: \"#{output}\"\n STDERR: \"#{error}\"")
        end
    ...
    def zip_info
      @zip_info ||= begin
        ...
        unless status.success?
          raise Error.new("Unzipping had errors\n STDOUT: \"#{output}\"\n STDERR: \"#{error}\"")
        end
